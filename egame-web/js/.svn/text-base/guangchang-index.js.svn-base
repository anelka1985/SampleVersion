...
function getNewData()
{
	...
	var interf = String.format(guangchang_dynamics_url,markDynamicInfoId,direction,userID);
...
function fillData(data)
{
	...
	if(data.result.resultcode=='0') {
		...
		markDynamicInfoId = 0;
		direction = 0;
		//wangfan 2012.3.7 start
		if(data.advertPic !== null && data.advertUrl !== null){
			var adPic = data.advertPic;
			$('#adImg').attr('src',adPic);			
			var adUrl = data.advertUrl;
			$('#ad').show();
			$('#ad_hide').show();
		...
		//end
		var homeUrl = String.format(guangchangIndex1_html,userID,markDynamicInfoId,direction);
		$("#pageHome").attr("href", homeUrl); 
		markDynamicInfoId = firstNum;
		direction = 1;
		var upUrl = String.format(guangchangIndex1_html,userID,markDynamicInfoId,direction);
		$("#pageUp").attr("href", upUrl);
		
		markDynamicInfoId = lastNum;
		direction = 2;
		downUrl = String.format(guangchangIndex1_html,userID,markDynamicInfoId,direction);
		$("#pageDown").attr("href", downUrl);
		
		markDynamicInfoId = 0;
		direction = 3;
		endUrl = String.format(guangchangIndex1_html,userID,markDynamicInfoId,direction);
