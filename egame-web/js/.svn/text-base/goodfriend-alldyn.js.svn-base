...
function getNewData()
{
	...
	if(friendId==null) showToast("friendId is null");
	$("#button").hide();
	markDynamicInfoId = 0;
	direction = 0;
	...
	var interf = String.format(guangchang_user_dyns_url,markDynamicInfoId,direction,userID,friendId);
	...
	if(data.result.resultcode=='0') {
	    ...
	    for (i in data.dynamicList)
		{
			...
			if(dynamic.commentNum>0) {
				commonNum="查看全部"+dynamic.commentNum+"条评论";
			}
			else {
				commonNum="暂无评论";
			}
		...
		markDynamicInfoId = 0;
		direction = 0;
		var homeUrl = String.format(goodfriend_alldyn_html,userID,markDynamicInfoId,direction,friendId);
		$("#pageHome").attr("href", homeUrl); 
		markDynamicInfoId = firstNum;
		direction = 1;
		var upUrl = String.format(goodfriend_alldyn_html,userID,markDynamicInfoId,direction,friendId);
		$("#pageUp").attr("href", upUrl);
		
		markDynamicInfoId = lastNum;
		direction = 2;
		downUrl = String.format(goodfriend_alldyn_html,userID,markDynamicInfoId,direction,friendId);
		$("#pageDown").attr("href", downUrl);
		
		markDynamicInfoId = 0;
		direction = 3;
		endUrl = String.format(goodfriend_alldyn_html,userID,markDynamicInfoId,direction,friendId);
